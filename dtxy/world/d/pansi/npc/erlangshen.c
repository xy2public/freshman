...
if (wx>=10000) wx=10000;
  ...
  {    
   ...
   message_vision(HIM"\n\n\n天边隐隐传来紫霞的声音："+who->query("name")+RANK_D->query_respect(who) + "灵台观礼之事，恕我不能前往道贺，望" + RANK_D->query_respect(who) + "代我向尊师菩提请罪了．\n\n\n\n"NOR,who);
		...
		who->delete("fangcun/panlong_nowaypansi");
 message("chat",HIC+"〖"+HIW+"大唐西游"+HIC+"〗如来佛祖(Rulai fo)："+who->query("name")+"助紫青二仙了却千年怨恨，皈依我佛，当真功德无量也．\n"NOR,users());
        message("chat",HIC+"〖"+HIW+"大唐西游"+HIC+"〗如来佛祖(Rulai fo)："+who->query("name")+"得到奖励：道行"+chinese_number(dx1)+"年，武学"+wx+"点，潜能30000点．\n"NOR,users());
	who->add("daoxing",dx);
