...
if (wx>=10000) wx=10000;
    ...
    if ((me->query_temp("fangcun/panlong_longgong_longzhu")=="done")&&(me->query_temp("fangcun/panlong_longgong_yuanshoucheng")=="done"))
	{
        ...
        me->delete_temp("fangcun/panlong_longgong_yuanshoucheng");
           message("chat",HIC+"〖"+HIW+"大唐西游"+HIC+"〗敖广(Ao guang)："+me->query("name")+"武功高强,见识不凡,观礼之日老龙必亲自前往道贺！\n"NOR,users());
              message("chat",HIC+"〖"+HIW+"大唐西游"+HIC+"〗敖广(Ao guang)："+me->query("name")+"得到奖励：道行"+chinese_number(dx1)+"年，武学"+wx+"点，潜能30000点．\n"NOR,users());
