...
if (wx>=10000) wx=10000;
	    ...
	    if ((me->query("fangcun/panlong_wzg_baxian")=="done")&&(me->query("fangcun/panlong_wzg_rsg")=="done"))
	{
       ...
       message("chat",HIC+"〖"+HIW+"大唐西游"+HIC+"〗镇元大仙(Zhenyuan daxian)："+me->query("name")+RANK_D->query_respect(me)+"了我八个徒孙心愿，他日灵台观礼老夫定来捧场！\n"NOR,users());
         message("chat",HIC+"〖"+HIW+"大唐西游"+HIC+"〗镇元大仙(Zhenyuan daxian)："+me->query("name")+"得到奖励：道行"+chinese_number(dx1)+"年，武学"+wx+"点，潜能30000点．\n"NOR,users());
